...
describe('Database - Replication', function () {
  ...
  //#region Issue 557. Please comment it before generic run
  /*describe('repo folder getting locked', () => {
    beforeEach(async () => {
      db1 = await Database({ ipfs: ipfs1, identity: testIdentity1, address: databaseId, accessController, directory: './orbitdb1' })
    })

    it('creating multiple OrbitDb instances with the same IPFS', async () => {
      let replicated = false
      let expectedEntryHash = null
      ...
      db2 = await Database({ ipfs: ipfs2, identity: testIdentity2, address: databaseId, accessController, directory: './orbitdb1' })
