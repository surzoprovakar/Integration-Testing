...
describe('Log', function () {
  ...
  //Ordering tie breaker can cause undefined ordering with the same identity
  describe('tie breaker same identity', () => {
    it('ordering tie breaker', async () => {
      const log1 = await Log(testIdentity, { logId: 'ABC' })
      const log2 = log1
      let values1 = await log1.values()
      let values2 = await log2.values()

      await log1.append('hello1')
      await log2.append('hello2')

      values1 = await log1.values()
      values2 = await log2.values()
      // console.log("values1 are: ", values1)
      // strictEqual(values1 instanceof Array, true)
      // strictEqual(values1.length, 2)
      // strictEqual(values2[0].payload, 'hello1')
      // strictEqual(values2[1].payload, 'hello2')
      strictEqual(values1, values2)
    })
  })
})
